...
不选中任何节点也可以启动核心，此时只有本地入站在工作，出站是 `freedom` 直连——用于「我只想要一个本机/局域网的 SOCKS/HTTP 代理口，不想走远程出站」的场景。

进入方式（二选一，效果相同）：

- **没有选中节点**：全新安装、节点列表为空、或选中的节点被删除后，直接点连接 FAB（或快捷设置磁贴 / 桌面小部件 / 快捷方式）即可。
- **显式切换**：主界面右上角「⋮」菜单 → **本地代理 · 直连**。这一项会把 `pref_local_proxy_direct_only` 置为 `true`，但**保留**当前选中的节点，方便一键切回；核心运行中选它会就地重启核心。切回节点只要在列表里点一下该节点。

生成的配置（其余部分与选中节点时完全一致）：
